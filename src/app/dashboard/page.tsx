...
import { DashboardStats, Activity, DailyStat } from "@/types/dashboard";
...
async function getStats(): Promise<DashboardStats> {
  ...
  // サーバーサイドでの直接データ取得に変更
  const { getUserLearningStats, getDailyStats } = await import("@/db/actions/dashboard");
...
async function DashboardContent() {
  ...
  return (
    <div className="space-y-8">
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <StatsCard
          title={<>総単語数 <span className="text-sm text-gray-500">Total Words</span></>}
          value={generalStats.totalWords}
          unit="単語"
        />
        <StatsCard
          title={<>完了した単語 <span className="text-sm text-gray-500">Completed Words</span></>}
          value={generalStats.completedWords}
          unit="単語"
        />
        <StatsCard
          title={<>進捗率 <span className="text-sm text-gray-500">Progress</span></>}
          value={Math.round(generalStats.progressPercentage)}
          unit="%"
        />
        <StatsCard
          title={<>クイズ正解率 <span className="text-sm text-gray-500">Quiz Accuracy</span></>}
      ...
      <div className="bg-white p-6 rounded-lg shadow-sm border">
        <h3 className="text-lg font-medium text-gray-900 mb-4 flex items-center gap-2">
          最近の学習活動
          <span className="text-sm text-gray-500">Recent Learning Activities</span>
        </h3>
            ...
            >
              <div>
                ...
                <p className="text-sm text-gray-500">
                  {activity.activityType === "quiz" ? "クイズ (Quiz)" : "復習 (Review)"}
      ...
      <div className="bg-white p-6 rounded-lg shadow-sm border">
        <h3 className="text-lg font-medium text-gray-900 mb-4 flex items-center gap-2">
          日別の学習統計
          <span className="text-sm text-gray-500">Daily Learning Statistics</span>
        </h3>
        <div className="space-y-4">
          {dailyStats.map((stat: DailyStat) => (
            ...
            >
              <div>
                ...
                <p className="text-sm text-gray-500">
                  クイズ (Quiz): {stat.quizCount} / 復習 (Review): {stat.reviewCount}
                </p>
              </div>
